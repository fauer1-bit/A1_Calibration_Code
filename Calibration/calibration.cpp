...
{
    // load the 3D model of the calibration rig
    const std::string rig_file = resource::directory() + "/data/corner.obj";
    if (!add_model(rig_file, true)) {
        LOG(ERROR) << "failed to load the calibration rig model: " << rig_file;
        exit(1);
    }
    ...
    // load the texture image of the calibration rig
    ...
    if (!tex) {
        LOG(ERROR) << "failed to load checkerboard texture: " << texture_file;
        exit(1);
    }
    ...
    opengl::prepare_cone(0.2, 20, vec3(base, 0, 0), vec3(base + head, 0, 0), vec3(1, 0, 0), points, normals, colors);
    opengl::prepare_cone(0.2, 20, vec3(0, base, 0), vec3(0, base + head, 0), vec3(0, 1, 0), points, normals, colors);
    opengl::prepare_cone(0.2, 20, vec3(0, 0, base), vec3(0, 0, base + head), vec3(0, 0, 1), points, normals, colors);
...
std::string Calibration::usage() const {
    return ("==================== Camera Calibration usage ====================\n"
            ...
            "------------------------------------------------------------------\n"
            "\tKey 'space': load 3D-2D correspondences and execute calibration \n"
            "------------------------------------------------------------------\n");
}
...
bool Calibration::open() {
    ...
    // create a drawable to visualize the 3D points
    for (const auto d : drawables_) {
        if (d->name() == "3D_points")
            delete_drawable(d);
    }
    auto drawable = new PointsDrawable("3D_points");
    ...
    drawable->set_default_color(vec4(1, 0, 0, 1));
...
bool Calibration::mouse_press_event(int x, int y, int button, int modifiers) {
    if (modifiers == GLFW_MOD_SHIFT) {
        ...
        if (found) {
            const Vector3D p3(std::round(p.x), std::round(p.y), std::round(p.z));
            points_3d_.push_back(p3);
            float scale = dpi_scaling();
            const Vector2D p2(std::round(x * scale), std::round(y * scale));
